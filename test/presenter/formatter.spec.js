...
import {
  ...
  sortGames,
  wrapCollection
} from '../../lib/presenter/formatter.js'
...
describe('formatGameName', async () => {
  test('formatGameName must return a shortter version of the name when the length is bigger than limit(15) characters', () => {
    const name =
      'Artificial Life 2061: Cybersys - (Fantasy Diva Of The Terrarian Vrchatworlds, Babel Tower Final Project: "Kodota Komori 1416") [Made by: Joseph Sanz]'
...
describe('sortGames', async () => {
  ...
  test('borked games have the 2th low priority for the sorting', () => {
    ...
    const borkedGame = createMergedGame({
      tier: 'borked',
      confidence: 'strong'
    })
    const silverGame = createMergedGame({
  ...
  test('bronze games have the 2th lower priority for the sorting', () => {
    ...
    const bronzeGame = createMergedGame({
      tier: 'bronze',
      confidence: 'strong'
    })
  ...
  test('platinum games have the 5th lower priority for the sorting', () => {
    ...
    const goldGame = createMergedGame({ tier: 'gold', confidence: 'strong' })
    const platinumGame = createMergedGame({
      tier: 'platinum',
      confidence: 'strong'
    })
    const games = [
  ...
  test('sorting several repeated tiers', () => {
    const game1 = createMergedGame({
      name: 'Counter-Strike',
      tier: 'gold',
      confidence: 'strong'
    })
    const game2 = createMergedGame({
      name: 'Counter-Strike: Source',
      tier: 'gold',
      confidence: 'strong'
    })
    const game3 = createMergedGame({
      name: 'Counter-Strike: Condition Zero',
      tier: 'platinum',
      confidence: 'strong'
    })
    const game4 = createMergedGame({
      name: 'Counter-Strike: Global Offensive',
      tier: 'gold',
      confidence: 'strong'
    })
    const game5 = createMergedGame({
      name: 'Graze Counter',
      tier: 'gold',
      confidence: 'strong'
    })
    const game6 = createMergedGame({
      name: 'Graze Counter GM',
      tier: 'platinum',
      confidence: 'strong'
    })
    const game7 = createMergedGame({
      name: 'Over the Counter',
      tier: 'GAME_NA',
      confidence: 'strong'
    })
    const game8 = createMergedGame({
      name: 'CounterAttack',
      tier: 'GAME_NA',
      confidence: 'strong'
    })
    const game9 = createMergedGame({
      name: 'Counterpack',
      tier: 'GAME_NA',
      confidence: 'strong'
    })
    const game10 = createMergedGame({
      name: 'Counterrealstic Syndrome',
      tier: 'GAME_NA',
      confidence: 'strong'
    })
    const game11 = createMergedGame({
      name: 'Counterrealstic Borked',
      tier: 'borked',
      confidence: 'strong'
    })
    const games = [
...
describe('wrapCollection', async () => {
  ...
  test('wrapCollection must return an array with a single element if the item concatenated length together is lower than the width', () => {
    ...
    assert.equal(
      wrappedCollection.length,
      1,
      'wrappedCollection has more than one item'
    )
  })
  ...
  test('wrapCollection must return an array with more than one element if the collection concatenated length is higher than the fixed width', () => {
    ...
    assert.equal(
      wrappedCollection.length,
      2,
      'wrappedCollection does not have the correct length'
    )
  })
})
...
describe('generateRequirementEntries', async () => {
  ...
  test('generateRequirementEntries must add elements to the array if there are requirements', () => {
    ...
    assert.deepEqual(result[0], [
      `{bold}${requirements.processor.title}{/bold}`,
      requirements.processor.text
    ])
    assert.deepEqual(result[1], [
      `{bold}${requirements.memory.title}{/bold}`,
      requirements.memory.text
    ])
  })

  test('generateRequirementEntries must add elements and keep the array if there are requirements', () => {
    const data = [['{bold}Videocard{/bold}', 'DirectX 9.0c compatible NVIDIA']]
    const requirements = {
      processor: {
        title: 'Processor',
        text: 'Core i7-6700 or Ryzen 5 1600'
    ...
    assert.deepEqual(result[0], [
      '{bold}Videocard{/bold}',
      'DirectX 9.0c compatible NVIDIA'
    ])
    assert.deepEqual(result[1], [
      `{bold}${requirements.processor.title}{/bold}`,
      requirements.processor.text
